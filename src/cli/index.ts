...
Commands:
  sync              Sync today's stats to the leaderboard
  leaderboard       Show leaderboard (default: all-time)
...
function parseArgs(args: string[]): Options {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "leaderboard") {
      options.leaderboard = "alltime";
      continue;
    }

    if (argument === "--day" || argument === "--week" || argument === "--month" || argument === "--alltime") {
      options.leaderboard = argument.replace("--", "");
      continue;
    }
...
async function main(): Promise<void> {
  ...
  if (options.leaderboard) {
    const { fetchLeaderboard } = await import("./sync");
    const { users, period } = await fetchLeaderboard(options.leaderboard);
    const muted = (text: string) => color("38;5;245", text);
    const highlight = (text: string) => color("1;38;5;255", text);
    const accent = (text: string) => color("38;5;81", text);
    const fire = (text: string) => color("38;5;208", text);

    console.log("");
    console.log(`  ${highlight("π pi-streak leaderboard")}  ${muted("—")}  ${highlight(period)}`);
    console.log("");

    const wRank = 4;
    const wUser = Math.max(...users.map((u) => u.username.length), 4);
    const wTokens = Math.max(...users.map((u) => formatTokens(u.tokens).length), "Tokens".length);
    const wStreak = Math.max(...users.map((u) => String(u.streak).length), "Strk".length);
    const wActive = Math.max(...users.map((u) => String(u.activeDays).length), "Days".length);

    const header = `${"Rank".padStart(wRank)} │ ${"User".padEnd(wUser)} │ ${"Tokens".padStart(wTokens)} │ ${"Strk".padStart(wStreak)} │ ${"Days".padStart(wActive)}`;
    const line = "─".repeat(wRank + wUser + wTokens + wStreak + wActive + 12);
    ...
    for (const u of users) {
      ...
      const userStr = `@${u.username}`.padEnd(wUser + 1);
      const tokensStr = formatTokens(u.tokens).padStart(wTokens);
      const streakStr = u.streak >= 30 ? fire(`${u.streak}🔥`.padStart(wStreak)) : String(u.streak).padStart(wStreak);
      const activeStr = String(u.activeDays).padStart(wActive);
      console.log(`  ${rankStr} │ ${highlight(userStr)} │ ${highlight(tokensStr)} │ ${streakStr} │ ${accent(activeStr)}`);
    }
